...
public class Board implements java.io.Serializable {
	...
	private String board_rename_filename;  //변경된 첨부파일이름
	private int board_ref;  //원글은 자기번호, 댓글은 원글번호
	private int board_reply_ref;  //원글 1, 댓글 2, 대댓글 3
	private int board_reply_lev;  //원글 0, 댓글은 자기번호, 대댓글은 참조하는 댓글번호
	private int board_reply_seq;  //댓글과 대댓글의 순번, 최근 댓글을 1
	private int board_readcount; //읽은 조회수
	private java.sql.Date board_date;  //게시글 등록 날짜
	...
	public Board(int board_num, String board_writer, String board_title, String board_content,
			...
			int board_reply_lev, int board_reply_seq, int board_readcount, Date board_date) {
		super();
