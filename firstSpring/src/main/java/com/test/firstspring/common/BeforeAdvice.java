...
//	private Logger logger = LoggerFactory.getLogger(BeforeAdvice.class());
	...
	//1. pointcut 설정용 메소드 작성 : 내용이 없어야함
	@Pointcut("execution(* com.test.firstspring..*.*(..))")
	public void allPointcut() {}
	
	//2. 작동시점과 적용할 포인트컷을 지정함
	//실행시킬 내용에 대한 코드 작성 및 실행
	@Before("allPointcut()")
	public void beforeLog(JoinPoint jp) {
		//포인트컷으로 지정한 타겟오브젝트의 메소드가 실행되기 전에 공통으로 작동될 기능에 대한 메소드임
		
		String methodName = jp.getSignature().getName();
		//타겟오브젝트(실제로 구현될)의 메소드 이름 추출함
		
		Object[] args = jp.getArgs();	//타겟 메소드의 전달인자 추출
		
		logger.info("[메소드 실행전 확인] : "+ methodName+"() 메소드 매개변수 개수 : "+args.length);
